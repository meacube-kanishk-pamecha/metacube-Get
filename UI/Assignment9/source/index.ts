...
class Vehicle {
    ...
    public vType: string;
    protected plateNumber: string;
    protected owner: Employee;

    constructor(vName: string, vType: string, plateNumber: string, owner: Employee) {
        this.vName = vName;
        this.vType = vType;
...
class Pass {
    ...
    constructor(validityDays: number, vehicle: Vehicle) {
    ...
    getPassDetails(): object {
        return { validUntil: this.validUntil, price: this.price, vehicle: this.vehicle.getVehicleDetails() };
    }
}

// Employee Form Handler
const empFormStart = (): void => {
    const frm = document.querySelector("#addEmployee form") as HTMLFormElement;
    frm.innerHTML = "";
    let gotName = "";
    ...
    const nxt = (): void => {
        ...
        frm.addEventListener("submit", (e: Event) => {
            ...
            if (f.type === "radio") {
                const ch = frm.querySelector(`input[name="${f.name}"]:checked`) as HTMLInputElement;
                val = ch ? ch.value : "";
                if (!val) return;
            } else {
                // if the type is password
                const inputElement = frm.querySelector(`input[name="${f.name}"]`) as HTMLInputElement;
...
// Pricing function
const showPriceList = (vType: string): void => {
    const cost: Record<string, number[]> = { "Cycle": [5, 100, 500], "MotorCycle": [10, 200, 1000], "Four Wheeler": [20, 500, 3500] };

    // document pricing element 
    const box = document.getElementById("pricing") as HTMLDivElement;
    ...
    btn.onclick = () => {
        ...
        let out = price;
        if (currency === "USD") out = parseFloat((price / 83).toFixed(2));
        else if (currency === "YEN") out = parseFloat((price * 1.57).toFixed(2));
...
const carForm= (emp: Employee): void => {
    const frm = document.querySelector("#addVehicle form") as HTMLFormElement;
    frm.innerHTML = "";
