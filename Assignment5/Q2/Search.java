...
public class Search {
  ...
  public int binaryFunc(int[] arr, int x, int l, int r) {
    // checking the condition of overflowing left should be less then right 
    if (r >= l && l <= arr.length - 1) {
      int idx = l + (r - l) / 2; //mid 
      if (arr[idx] == x)
        return idx;
        // if value at idx is greater then 
      if (arr[idx] > x)
        return binaryFunc(arr, x, l, idx - 1);
        // 
      else
        return binaryFunc(arr, x, idx + 1, r);
    }
    return -1;
  }
